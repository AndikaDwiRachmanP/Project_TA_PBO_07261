...
public class CustomerModel extends ModelAbstract {
    ...
    public void update(CustomerEntity customer) {
        ...
        try {
            ...
            String sql = "UPDATE customer t set t.nama = '%s', t.alamat = '%s', t.no_hp = '%s', t.password = '%s', t.updated_at = '%s' where t.id = %d";
            sql = String.format(sql, customer.getNama(), customer.getAlamat(), customer.getNo_hp(), customer.getPassword(), Komponen.getCurrentTime(), customer.getId());
    ...
    public CustomerEntity getName (int index) {
        ArrayList<CustomerEntity> customer = getData();
        CustomerEntity nama;

        for(CustomerEntity i: customer) {
            if (index == i.getId()) {
                nama = i;
                return nama;
            }
